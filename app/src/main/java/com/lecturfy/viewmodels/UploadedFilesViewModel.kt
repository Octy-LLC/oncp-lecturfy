...
data class UploadedfilesViewModelState (
  val transcriptions: List<TranscriptionspaginationEntity> = listOf(
  ),
...
@HiltViewModel
class UploadedfilesViewModel @Inject constructor (
  ...
  private val _state = MutableStateFlow<UploadedfilesViewModelState>(UploadedfilesViewModelState());
  ...
  fun init() {
    ...
    var transcriptions = transcriptionsgetTranscriptions(
      authorizationHeaderParam=accessTokenLocal,
    )
    if(transcriptions == null) return@launch  
    
    setTranscriptions(transcriptions.body()!!)
...
fun setTranscriptions(list: List<TranscriptionspaginationEntity>) {
  _state.update { currentState ->
    currentState.copy(transcriptions = list)
  }
}
